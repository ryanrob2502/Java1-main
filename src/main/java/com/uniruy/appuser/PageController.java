...
public class PageController {
    ...
    public ResponseEntity<?> getDistribuicaoIdade(HttpSession session) {
        ...
        try {
            ...
            distribuicao.put("0-18",
                    pacientes.stream().filter(p -> p.getIdade() != null && p.getIdade() <= 18).count());
            distribuicao.put("19-35", pacientes.stream()
                    .filter(p -> p.getIdade() != null && p.getIdade() > 18 && p.getIdade() <= 35).count());
            distribuicao.put("36-50", pacientes.stream()
                    .filter(p -> p.getIdade() != null && p.getIdade() > 35 && p.getIdade() <= 50).count());
            distribuicao.put("51-65", pacientes.stream()
                    .filter(p -> p.getIdade() != null && p.getIdade() > 50 && p.getIdade() <= 65).count());
